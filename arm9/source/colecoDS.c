...
u16	xfer_buf[10000]	ALIGN(32)	=	{0};
u16	xfer_buf_ay[10000] ALIGN(32) = {0};
...
mm_word	VblSound2(mm_word	len, mm_addr dest, mm_stream_formats format)
{
	if	(soundEmuPause)	memset(dest, 0x00, len*4);
	else
	{
		if (AY_Enable)
    {
      ...
      {
        *p++ = (u16) (((u32)mixbuf1[i] + (u32)mixbuf2[i]) >> 1);
      }
    }
		else sn76496Mixer(len*4, dest, &sncol);
	}
	return	len;
}
